...
const AvatarGroup: Component<AvatarGroupProps> = (props) => {
  ...
  return (
    <div class={cn("flex items-center -space-x-1.5", local.class)} {...others}>
      <For each={visible()}>
        {(item) => (
          <Show
            when={item.image}
            fallback={
              <div
                class={cn(
                  "flex items-center justify-center rounded-full border-2 border-background bg-surface-muted font-medium text-surface-muted-foreground ring-1 ring-border-subtle",
                  sizeClasses[size()],
          ...
          >
            <img
              ...
              title={item.name}
              class={cn(
                "rounded-full border-2 border-background ring-1 ring-border-subtle object-cover",
                sizeClasses[size()],
              )}
            />
          </Show>
      ...
      <Show when={overflow() > 0}>
        <div
          class={cn(
            "rounded-full border-2 border-background ring-1 ring-border-subtle flex items-center justify-center bg-muted text-muted-foreground font-medium",
            sizeClasses[size()],
            textSizeClasses[size()],
          )}
        >
