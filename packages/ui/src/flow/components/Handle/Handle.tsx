...
export function Handle(allProps: HandleProps) {
	...
	createRenderEffect(
		...
		if (props.onconnect || props.ondisconnect) {
			...
			store.edges;
			return store.connectionLookup.get(
		...
		if (!connections) return;
			...
			if (
				prevConnections &&
				!areConnectionMapsEqual(connections, prevConnections)
			) {
	...
	function onConnectExtended(connection: Connection) {
		const edge = store.onbeforeconnect
			? store.onbeforeconnect(connection)
			: connection;
		if (!edge) return;
		store.addEdge(edge);
		...
		if (
			...
			XYHandle.onPointerDown(event, {
				...
				isValidConnection:
					...
					((...args) => store.isValidConnection?.(...args) ?? true),
	...
	function onclick(event: MouseEvent) {
		...
		const isValidConnectionHandler =
			props.isValidConnection ?? store.isValidConnection;

		const { connectionMode, clickConnectStartHandle, flowId, nodeLookup } =
			store;
