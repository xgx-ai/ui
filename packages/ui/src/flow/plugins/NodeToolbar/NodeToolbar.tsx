...
export function NodeToolbar(allProps: NodeToolbarProps) {
	...
	const toolbarNodes = createMemo(() => {
		...
		const nodeIds = Array.isArray(props.nodeId)
			? props.nodeId
			: [props.nodeId ?? contextNodeId];
		return nodeIds.reduce<InternalNode[]>((res, nodeId) => {
			if (!nodeId)
				throw new Error(
					"Either pass a nodeId or use within a Custom Node component",
				);
	...
	const selectedNodesCount = createMemo(
		() => store.nodes.filter((n) => n.selected).length,
	);
