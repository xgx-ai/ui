...
export function ResizeControl(allProps: ResizeControlProps) {
	...
	const id = () =>
		typeof props.nodeId === "string" ? props.nodeId : contextNodeId;

	const [resizeControlRef, setResizeControlRef] =
		createSignal<HTMLDivElement | undefined>();
	let resizer: XYResizerInstance | null = null;

	const isLineVariant = () =>
		(props.variant ?? ResizeControlVariant.Handle) ===
		ResizeControlVariant.Line;

	const controlPosition = createMemo<ControlPosition>(() => {
		const defaultPosition = (
			isLineVariant() ? "right" : "bottom-right"
		) as ControlPosition;
	...
	createRenderEffect(() => [resizeControlRef(), id()] as const, ([resizeControl, nodeId]) => {
		...
		const instance = XYResizer({
			...
			onChange: (
				...
				const horizontal =
					!props.resizeDirection || props.resizeDirection === "horizontal";
				const vertical =
					!props.resizeDirection || props.resizeDirection === "vertical";
	...
	return (
		<div
			...
			style={{
				...
				scale:
					isLineVariant() || !autoScale()
						? undefined
						: `${Math.max(1 / store.viewport.zoom, 1)}`,
			}}
			{...rest}
		>
			{props.children}
