...
import { createEffect, createRenderEffect, createSignal, onSettled, untrack } from "solid-js";
 ...
 */
export function SolidFlow<NodeType extends Node = Node, EdgeType extends Edge = Edge>(
  ...
  //
  // createEffect, not createRenderEffect: these callbacks are host code, and the whole
  // point of onselectionchange is that the host records what is selected. A render
  // effect's apply phase is an owned scope, so a setState in there raises
  // REACTIVE_WRITE_IN_OWNED_SCOPE, which halts the entire reactive system — the flow
  // stops responding to clicks, drags and connections from that moment on.
  createEffect(
  ...
  return (
    <StoreContextObj value={{ provider: false, getStore: () => store }}>
      <div
        ref={setWrapperRef}
        style={{
