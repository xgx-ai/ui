...
function themeEncoding(encoding: unknown) {
  ...
  if (isRecord(color)) {
    if (typeof color.value === "string") {
      ...
      const hasExplicitRange = Array.isArray(scale.range) && scale.range.length > 0;
      next.color = {
        ...color,
        scale: {
          ...scale,
...
function themeConfig(config: unknown) {
  ...
  return {
    ...
    range: {
      ...range,
      category: Array.isArray(range.category) ? range.category : AMA_CHART_PALETTE,
...
function applyAmaChartTheme(spec: Record<string, unknown>): Record<string, unknown> {
  ...
  if ("mark" in spec) {
    next.mark = themeMark(spec.mark, isRecord(encoding) && isRecord(encoding.color));
  }

  if (Array.isArray(spec.layer)) {
    next.layer = spec.layer.map((layer) => (isRecord(layer) ? applyAmaChartTheme(layer) : layer));
  }

  return next;
}

function findDirectBlockElementAtPoint(view: EditorView, clientX: number, clientY: number) {
...
function resolveDropTarget(view: EditorView, clientX: number, clientY: number) {
...
function moveNodeToPosition(view: EditorView, getPos: () => number | undefined, insertPos: number) {
  ...
  if (insertPos >= currentPos && insertPos <= currentPos + chartNode.nodeSize) {
    return;
  }

  const tr = state.tr.delete(currentPos, currentPos + chartNode.nodeSize);
  const adjustedInsertPos = insertPos > currentPos ? insertPos - chartNode.nodeSize : insertPos;
...
export const VegaLiteChart = Node.create({
  ...
  renderHTML({ HTMLAttributes }) {
    return ["div", mergeAttributes(HTMLAttributes, { "data-type": "vega-lite-chart" }), 0];
  },
  ...
  addNodeView() {
    return ({
      ...
      chart.className =
        "overflow-auto w-full [&>div]:w-full [&_svg]:w-full [&_svg]:h-auto [&_*]:!cursor-pointer";

      const error = document.createElement("div");
      error.className = "hidden text-sm text-muted-foreground italic py-2";
      ...
      const renderChartImmediate = async () => {
        ...
        try {
          ...
          clearChart();
          error.textContent = e instanceof Error ? `Invalid JSON: ${e.message}` : "Invalid JSON.";
          error.classList.remove("hidden");
          return;
        }
        ...
        try {
          ...
          clearChart();
          error.textContent =
            e instanceof Error ? `Chart render failed: ${e.message}` : "Chart render failed.";
          error.classList.remove("hidden");
        }
      };
      ...
      const showDropIndicator = (target: { left: number; top: number; width: number }) => {
      ...
      const handlePointerMove = (event: PointerEvent) => {
        ...
        pointerTracking.dropTarget = dropTarget ? { insertPos: dropTarget.insertPos } : null;
        if (dropTarget) {
      ...
      const finishPointerDrag = (event: PointerEvent) => {
        ...
        const dropTarget =
          storedDropTarget ?? resolveDropTarget(view, event.clientX, event.clientY);
        if (!dropTarget) {
          return;
        }
