...
function ActivityTimeline<TItem = unknown>(
	props: ActivityTimelineProps<TItem>,
) {
	...
	const showFilters = () =>
		local.filters && local.filters.length > 0;
	const showSearch = () => local.onSearchChange !== undefined;
	const showToolbar = () =>
		showFilters() || showSearch() || local.headerActions;
	...
	return (
		<div
			class={cn(
				"bg-white h-full w-full p-4 rounded-lg border overflow-hidden flex flex-col",
				local.class,
			)}
			{...others}
		>
							...
							<div class="flex gap-1 h-full items-center">
								<For each={local.filters}>
									{(filter) => (
										<Button
											variant={
												local.selectedFilter === filter.id
													? "default"
													: "outline"
											}
											size="sm"
											...
											onClick={() =>
												local.onFilterChange?.(filter.id)
											}
										>
											{filter.label}
										</Button>
			...
			<div
				class={cn(
					"flex-1 overflow-y-auto flex flex-col gap-5 bg-muted/30 p-4 rounded-md",
					showToolbar() && "mt-3",
				)}
			>
				<Show
